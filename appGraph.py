...
def add_differential_privacy(data, epsilon=1.0, scaling_factor=1000):
    ...
    for column in data.columns:
        if pd.api.types.is_numeric_dtype(data[column]):
            # 大きすぎる数値を除外し、NaNを除去
            valid_data = data[column].dropna()[(data[column] < 1e6) & (data[column] > -1e6)]
            
            # データを整数にスケーリング
            scaled_data = (valid_data * scaling_factor).astype(int)
            ...
            # 元のスケールに戻す
            noisy_data[column].loc[valid_data.index] = [val / scaling_factor for val in noisy_values]
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # スケーリングファクターを入力
        scaling_factor = st.number_input("スケーリングファクターを入力してください", min_value=1, max_value=1000000, value=1000, step=100)
        ...
        if 'noisy_data' not in st.session_state or st.button("ノイズを加える"):
            ...
            st.session_state.noisy_data = add_differential_privacy(numeric_data, epsilon, scaling_factor)
        ...
        for epsilon in epsilon_values:
            noisy_data = add_differential_privacy(numeric_data, epsilon, scaling_factor)
